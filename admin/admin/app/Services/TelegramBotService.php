...
class TelegramBotService
{
    ...
    public function setMyCommands($commands = null, $scope = null, $languageCode = null)
    {
        ...
        // Telegram Bot API 要求 commands 参数是 JSON 字符串格式
        // 构建 POST 数据，commands 需要是 JSON 字符串
        $postData = [
        ...
        if ($scope !== null) {
            if (is_string($scope) && $scope === 'default') {
                ...
                $postData['scope'] = json_encode($scope);
            }
        }
        ...
        try {
            // 构建 POST 数据字符串，确保 commands 参数正确编码
            $commandsJson = json_encode($commands);
            $postFields = 'commands=' . urlencode($commandsJson);
            
            // 如果提供了scope，添加到请求中
            if ($scope !== null && is_array($scope)) {
                $postFields .= '&scope=' . urlencode(json_encode($scope));
            }
            
            // 如果提供了语言代码，添加到请求中
            if ($languageCode !== null) {
                $postFields .= '&language_code=' . urlencode($languageCode);
            }
            
            Log::info('设置Bot命令菜单 - 发送请求', [
                'commands' => $commands,
                'commands_json' => $commandsJson,
                'post_fields' => $postFields,
                'api_url' => $this->apiUrl . 'setMyCommands'
            ]);
            ...
            if (!$result || !isset($result['ok']) || !$result['ok']) {
                ...
                return ['code' => 500, 'message' => '设置Bot命令菜单失败：' . ($result['description'] ?? '未知错误'), 'data' => $result];
            }
    ...
    {
        // Telegram API 不允许消息文本为空，如果为空则使用零宽非断字符
        // 注意：Telegram API 在验证时会 trim 文本，所以空格也会被认为是空的
        if (empty(trim($text))) {
            $text = "\u{FEFF}";  // 使用零宽非断字符 (U+FEFF)，不可见但满足API要求
        }
