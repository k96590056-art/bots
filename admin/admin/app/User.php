...
class User extends Authenticatable
{
    ...
    {
        // 获取当前代理及其所有下级代理的ID
        $child = self::getAllChildIds($userid);
    ...
    /**
     * 获取所有下级ID（递归，不依赖系统配置层级限制）
     */
    public static function getAllChildIds($user_id)
    {
        $ids = [];
        $children = self::where(function($query) use ($user_id) {
            $query->where('pid', $user_id)
                  ->orWhere('fid', $user_id);
        })->pluck('id');
        ...
        if ($children->isNotEmpty()) {
            foreach ($children as $child_id) {
                $ids[] = $child_id;
                $ids = array_merge($ids, self::getAllChildIds($child_id));
            }
        }
    ...
    public static function getChildNext($user_id)
    {
        ...
        if (!$user) {
            ...
            foreach ($user as $k => $v) {
                $arr[] = $v;
                $arr = array_merge($arr, self::getAllChildIds($v));
            }
        }
        return $arr;
    }
    ...
    {
        $child = self::getAllChildIds($user_id);
    ...
    {
        $child = self::getAllChildIds($this->id);
