...
class TelegramWebhookController extends Controller
{
    ...
    {
        $cacheKey = 'telegram_bot_commands_initialized';
        
        // 检查是否已经设置过（缓存24小时）
        if (!Cache::has($cacheKey)) {
            Log::info('开始设置Bot命令菜单');
            $result = $this->telegramBot->setMyCommands();
            if ($result['code'] == 200) {
                // 设置成功后缓存24小时
                Cache::put($cacheKey, true, now()->addHours(24));
                Log::info('Bot命令菜单设置成功', ['result' => $result]);
            } else {
                Log::error('Bot命令菜单设置失败', [
                    'result' => $result,
    ...
    public function webhook(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                // 初始化失败不应该阻止webhook处理
                Log::warning('Bot命令初始化失败', ['error' => $initError->getMessage()]);
            }
            ...
            // 记录异常日志（使用Throwable捕获所有错误，包括Fatal Error）
            try {
    ...
    protected function handleMessage($message)
    {
        ...
        // 处理常驻键盘菜单按钮点击
        // 注意：第一个按钮（进入游戏）是web_app类型，不会触发文本消息，而是直接打开web app
        switch ($text) {
            case '💰 账户余额':
                // 显示账户余额信息
    ...
    protected function getPersistentKeyboard()
    {
        ...
        return [
            // 第一行：进入游戏按钮（web_app类型，点击直接打开游戏页面）
            [
                [
                    'text' => '🎮 游戏入口',
                    'web_app' => [
     ...
     * 参数：action=set_commands 可以手动设置命令菜单
     *
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function debug(Request $request)
    {
        try {
            ...
            if ($action === 'set_commands') {
                // 清除缓存，强制重新设置
                Cache::forget('telegram_bot_commands_initialized');
                $result = $this->telegramBot->setMyCommands();
                return response()->json([
                    'success' => $result['code'] == 200,
                    'action' => 'set_commands',
                    'result' => $result,
                    'message' => $result['code'] == 200 ? '命令菜单设置成功' : '命令菜单设置失败：' . ($result['message'] ?? '未知错误')
                ]);
            }
            ...
            $debugInfo = [
                ...
                'test_time' => date('Y-m-d H:i:s'),
                'cache_status' => Cache::has('telegram_bot_commands_initialized') ? '已设置（缓存中）' : '未设置',
            ];
            ...
            if (!empty($botToken)) {
                ...
                try {
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, 'https://api.telegram.org/bot' . $botToken . '/getMyCommands');
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 10);
                    curl_setopt($ch, CURLOPT_TIMEOUT, 10);
                    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
                    ...
                    $response = curl_exec($ch);
                    curl_close($ch);
                    $commandsInfo = json_decode($response, true);
                    $debugInfo['commands_info'] = $commandsInfo;
                } catch (\Exception $e) {
                    $debugInfo['commands_check_error'] = $e->getMessage();
                }
            }
