...
class Dbdianzi extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'dbdianzi {param?}';
     ...
     * @var string
     */
    protected $description = 'Dianzi游戏数据同步命令';
    ...
    public function handle()
    {
        ...
        if (empty($param)) {
            ...
            $this->info('示例：');
            $this->info('  php artisan dbdianzi 1                 # 同步1分钟内的游戏记录');
            $this->info('  php artisan dbdianzi balance           # 同步用户余额');
            $this->info('  php artisan dbdianzi syncBalance       # 同步用户余额（兼容写法）');
            return;
        }
    ...
    private function syncBalance()
    {
        ...
        $platformNames = GameList::where('with_api', 'dbdianzi')
        ...
        if (empty($platformNames)) {
            $this->warn('game_lists 未配置 with_api=dbdianzi，无法定位平台列表');
            return;
        }
