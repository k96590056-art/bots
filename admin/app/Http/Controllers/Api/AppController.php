...
class AppController extends Controller
{
    ...
    public function api_login(Request $request)
    {
        ...
        $gameItem = GameList::where('platform_name', $api_code)->where('game_code', $gameCode)->first();
        if ($gameItem && (((int)$gameItem->site_state !== 1) || ((int)$gameItem->app_state !== 1))) {
            return $this->returnMsg(500, '', '该游戏已关闭');
        }
        $withApi = $gameItem->with_api ?? 'tg';
        ...
        if ($withApi === 'tg') {
            ...
            if(!$User_Api){
                ...
                // 注册成功后，user_api表记录已自动创建，无需手动创建
            }
            $res = $service->login($user->username, $gameCode, 1);
        } else {
