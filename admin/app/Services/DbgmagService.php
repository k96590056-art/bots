...
class DbgmagService
{
    ...
    public function login($username, $api_code = '', $is_mobile = 2, $game_code = '', $game_type = '1')
    {
        ...
        // 确定平台类型
        $platform = $is_mobile == 2 ? 'mobile' : 'web';
        ...
        Log::error('Dbgmag登录成功（POST方式）', $this->err);
        // POST 方式返回的是纯文本URL
     ...
     *
     * URL: https://{gmag_game_data_url}/game/list?hash=...
     * Method: POST
     *
     * 可用于按 providerCode / gameType / gameCode 搜索游戏信息。
     *
     * @param string $providerCode 游戏供应商编码（可选）
     * @param string $gameType 游戏类型（可选，如 slots/table/live/...）
     ...
     */
    public function getGameList($providerCode = '', $gameType = '', $gameCode = '', $page = 1, $size = 100)
    {
        $return = [
            'code' => 200,
            'message' => '成功',
