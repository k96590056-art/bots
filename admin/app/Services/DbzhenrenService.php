...
class DbzhenrenService
{
    ...
    public function __construct()
    {
        // 配置参数（写死，不再从数据库读取）
        $this->db_code = "DBZR";
        $this->api_url = 'https://api.obothapi.com';  // 游戏接口地址
        $this->api_data_url = 'https://api-data.obothapi.com';  // 游戏数据接口地址
        $this->merchant_code = 'QBEPFA';  // 商户代码
        $this->secret_key = 'xKqXwds3HuUAhOWK';  // MD5签名密钥
        $this->aes_key = '5hIbr2QIxbEPq7Nb';  // AES加密密钥
        $this->md5_key = "xKqXwds3HuUAhOWK";  // MD5 Key
    }

    /**
     * 获取商户前缀（用于去除 playerName 前缀后匹配本地用户）
     */
    public function getMerchantCode(): string
    {
        return (string)$this->merchant_code;
    }
     ...
     * 
     * 适配控制器调用方式：register($api_code, $username)
     * 兼容原有调用方式：register($loginName, $loginPassword, $api_code = "", ...)
     * 
     * @param string $loginName 游戏账号（6-50字符，需要包括商户的前缀，只能包含以下特殊字符[下划线、@、#、&、*]）
     *                         或 $api_code（当第二个参数是字符串且长度>6时，视为控制器调用方式）
     * @param string $loginPassword 登陆密码（6-32字符，不允许的符号：`''[]./'"'$）
     *                             或 $username（当第一个参数是api_code时）
     ...
     */
    public function register($api_code = "", $loginName, $loginPassword = '123456', $nickName = '', $oddType = 0, $lang = 1, $version = 'v2')
    {
        ...
        // 适配控制器调用方式：register($api_code, $username)
        // 判断：如果 $loginPassword 是字符串且长度>6，且 $api_code 为空，可能是控制器调用方式
        // 更准确的判断：如果第二个参数看起来像用户名（长度>=6且不包含特殊字符），且第一个参数长度较短，则视为控制器调用
        if (strlen($loginPassword) >= 6 && 
            strlen($loginPassword) <= 50 && 
            empty($api_code) && 
            preg_match('/^[a-zA-Z0-9_@#&*]+$/', $loginPassword)) {
            // 控制器调用方式：register($api_code, $username)
            $username = $loginPassword;
            $loginPassword = '123456';  // 默认密码
        } else {
            // 原有调用方式：register($loginName, $loginPassword, ...)
            $username = $loginName;
        }
     ...
     * 4.9 进入游戏
     * 
     * 适配控制器调用方式：login($username, $api_code, $game_type, $is_mobile, $game_code)
     * 兼容原有调用方式：login($loginName, $loginPassword, $api_code = "", $deviceType = 2, ...)
     * 
     * @param string $loginName 游戏账号或 $username（控制器调用时）
     * @param string $loginPassword 游戏密码或 $api_code（控制器调用时）
     * @param string $api_code API代码（可选）或 $game_type（控制器调用时）
     * @param int $deviceType 设备类型（1=PC, 2=H5, 3=iOS, 4=Android）或 $is_mobile（控制器调用时，0=PC，1=手机）
     * @param int $oddType 盘口类型（V1版本必填，V2版本已取消）或 $game_code（控制器调用时，游戏代码）
     * @param int $lang 语言（固定设为1）
     * @param string $backurl 返回商户地址（可选）
     * @param string $domain 动态游戏域名（可选）
     * @param int $showExit 是否显示退出按钮（0=显示，1=不显示）
     ...
     * @return array
     */
    public function login($loginName, $loginPassword = '123456', $api_code = "", $deviceType = 2, $gameTypeId = "", $oddType = 0, $lang = 1, $backurl = '', $domain = '', $showExit = 0, $anchorId = 0, $ip = '', $isCompetition = 0, $playerLanguageV2 = '', $version = 'v2')
    {
        if (empty($this->api_url)) {
            return [
                'code' => 400,
        ...
        // 适配控制器调用方式：login($username, $api_code, $game_type, $is_mobile, $game_code)
        // 判断逻辑：如果第二个参数看起来像api_code（短字符串），第三个参数是数字（game_type），第四个参数是0或1（is_mobile）
        $isControllerCall = false;
        if (strlen($loginPassword) <= 20 && 
            !empty($api_code) && 
            is_numeric($api_code) && 
            ($deviceType == 0 || $deviceType == 1)) {
            // 控制器调用方式：login($username, $api_code, $game_type, $is_mobile, $game_code)
            $isControllerCall = true;
            $username = $loginName;
            $actualPassword = '123456';  // 默认密码
            $game_type = $api_code;  // 第三个参数是game_type（leixing）
            $is_mobile = $deviceType;  // 第四个参数是is_mobile（0=PC, 1=手机）
            $game_code = $gameTypeId;  // 第五个参数是game_code
            
            // 转换设备类型：0=PC, 1=手机 -> 1=PC, 2=H5
            $deviceType = $is_mobile ? 2 : 1;
            
            // 如果提供了game_code，转换为gameTypeId
            if (!empty($game_code) && is_numeric($game_code)) {
                $gameTypeId = (int)$game_code;
            } else {
                $gameTypeId = '';
            }
        } else {
            // 原有调用方式：login($loginName, $loginPassword, ...)
            $username = $loginName;
            $actualPassword = $loginPassword;
        }
        ...
        if (empty($backurl)) {
            $isMobile = ($deviceType == 2 || $deviceType == 3 || $deviceType == 4);
            if ($isMobile) {
                $backurl = env('WAP_URL', '');
            } else {
                $backurl = env('PC_URL', '');
        ...
        $params = [
            'loginName' => $fullLoginName,
            'loginPassword' => isset($actualPassword) ? $actualPassword : $loginPassword,
            'deviceType' => $deviceType,
            'timestamp' => time() * 1000,
        ];
    ...
    public function balance($api_code, $username, $password = '123456')
    {
        ...
        /**
         * 从 user_api 表获取余额（回调钱包/本地钱包）
         *
         * 重要：控制器/命令里传入的 $api_code 通常是“真实场馆 platform_name”，
         * user_api.api_code 也对应存 platform_name。
         * 之前这里固定用 $this->db_code（如 dbzhenren）去查，会导致查不到记录，回收时误判为 0。
         */
        $apiCode = strtolower(trim((string)$api_code));
        $candidates = array_values(array_unique(array_filter([
            $apiCode,
            strtolower(trim((string)$this->db_code)),
            strtolower(trim((string)$this->merchant_code)), // 兼容部分旧数据可能把商户号写进 api_code
        ])));

        $userApi = User_Api::where('api_user', $username)
            ->whereIn('api_code', $candidates)
            ->orderBy('id', 'desc')
            ->first();

        if ($userApi) {
            $balance = $userApi->api_money ?? 0;
            $return['data'] = round($balance, 4);
            return $return;
        }

        // 如果 user_api 记录不存在，返回0
        $return['data'] = 0;
        Log::info('Dbzhenren查询余额 - user_api记录不存在', [
            'username' => $username,
            'api_code' => $api_code
        ]);
    ...
    /**
     * 查询玩家官方余额（走真人商户接口，不读本地 user_api.api_money）
     * 用于命令行 syncBalance：以官方余额为准写回 user_api.api_money
     *
     * 说明：此接口路径在不同版本文档可能略有差异，这里按常见写法实现：
     * - POST https://{api_domain}/api/merchant/balance/v1
     * 入参常见字段：loginName, timestamp
     *
     * 若你们文档实际路径/字段不同，我可以按你给的文档再精确调整。
     */
    public function balanceOfficial($api_code, $username, $password = '123456')
    {
        $return = [
            'code' => 200,
            'message' => '成功',
        ...
        // 在用户名前面加上 merchant_code 前缀
        $fullLoginName = (string)$username;
        if (!empty($this->merchant_code) && strpos($fullLoginName, $this->merchant_code) !== 0) {
            $fullLoginName = $this->merchant_code . $fullLoginName;
        }
        ...
        // 常见“余额查询”接口路径
        $url = $this->getApiUrl('/api/merchant/balance/v1');
        $res = $this->sendRequest($url, $params, 'POST', 'application/json', [], true);

        if (isset($res['code']) && (int)$res['code'] === 200) {
            // 常见返回：data.balance
            $balance = $res['data']['balance'] ?? ($res['data']['money'] ?? null);
            if ($balance === null && isset($res['data'])) {
                // 兜底：如果 data 直接是数值
                $balance = is_numeric($res['data']) ? $res['data'] : null;
            }
            $return['data'] = $balance !== null ? round((float)$balance, 4) : 0;
            return $return;
        }

        $return['code'] = 201;
        $return['message'] = $res['message'] ?? ('查询官方余额失败（code=' . (string)($res['code'] ?? 'unknown') . '）');
        $return['data'] = 0;
        $return['raw'] = $res;

        Log::warning('Dbzhenren查询官方余额失败', [
            'username' => $username,
            'full_login_name' => $fullLoginName,
            'api_code' => $api_code,
            'url' => $url,
            'response' => $res,
        ]);
     ...
     * 
     * 注意：真人平台使用回调机制，这里只记录转账记录，实际转账由平台回调处理
     * 
    ...
    public function deposit($username, $amount, $ext_trans_id, $api_code = '')
    {
        ...
        $params = [
            'loginName'   => $fullLoginName,
            'transferNo'  => (string)$ext_trans_id,
            'amount'      => $amount,
            'showBalance' => 1, // 返回余额
            'timestamp'   => (int)round(microtime(true) * 1000),
        ];

        $url = $this->getApiUrl('/api/merchant/deposit/v1');
        $res = $this->sendRequest($url, $params, 'POST', 'application/json', [], true);
        ...
        if (isset($res['code']) && (int)$res['code'] === 200) {
            ...
            $return['data'] = [
                'transId'    => $res['data']['transId'] ?? $ext_trans_id,
        ...
        // 失败或非200时，按文档调用 4.6 转账结果查询接口兜底
        $queryRes = $this->queryTransferResult($fullLoginName, $ext_trans_id);
        if (isset($queryRes['code']) && (int)$queryRes['code'] === 200) {
            // 如果查询确认成功，则按成功返回
            $return['data'] = [
                'transId'    => $queryRes['data']['transId'] ?? $ext_trans_id,
                'extTransId' => $ext_trans_id,
                'status'     => $queryRes['data']['status'] ?? 'approved',
                'balance'    => isset($queryRes['data']['balance']) ? round((float)$queryRes['data']['balance'], 2) : null,
                'raw'        => $queryRes['data'] ?? [],
            ];
            $return['message'] = $queryRes['message'] ?? '成功';
            return $return;
        }
        ...
        Log::error('Dbzhenren上分失败', [
            ...
            'ext_trans_id' => $ext_trans_id,
            'response' => $res,
            'query_transfer_result' => $queryRes ?? null
        ]);
    ...
    public function withdrawal($username, $amount, $ext_trans_id, $api_code = '')
    {
        ...
        $url = $this->getApiUrl('/api/merchant/withdraw/v1');
        $res = $this->sendRequest($url, $params, 'POST', 'application/json', [], true);
    ...
    public function checkTrans($ext_trans_id)
    {
        ...
        // 从数据库中查询转账记录
        $transferLog = \App\Models\TransferLog::where('order_no', $ext_trans_id)->first();
        
        if ($transferLog) {
            $return['data'] = [
                'extTransId' => $ext_trans_id,
                'status' => $transferLog->state == 1 ? 'approved' : 'pending',
                'amount' => $transferLog->real_money ?? 0,
            ];
        } else {
            $return['code'] = 201;
            $return['message'] = '交易记录不存在';
        }
    ...
    /**
     * 4.6 转账交易结果查询接口（兜底）
     * 文档未给出具体路径，这里按惯例使用 /api/merchant/queryTransfer/v1。
     * 如果后续文档有明确路径，请据此调整。
     *
     * @param string $loginName 带前缀的登录名
     * @param string $transferNo 交易单号
     * @return array
     */
    private function queryTransferResult($loginName, $transferNo)
    {
        $params = [
            'loginName'   => $loginName,
        ...
        $url = $this->getApiUrl('/api/merchant/queryTransfer/v1');
        $res = $this->sendRequest($url, $params, 'POST', 'application/json', [], true);
    ...
    {
        // 转换时间格式为 yyyy-MM-dd HH:mm:ss（避免重复替换导致 10:50:00:00 的错误格式）
        $startTime = $this->normalizeToYmdHis($start_time);
        $endTime = $this->normalizeToYmdHis($end_time);

        if (empty($startTime) || empty($endTime)) {
            return [
                'code' => 201,
                'message' => '时间格式错误，应是yyyy-MM-dd HH:mm:ss格式'
            ];
        }

        return $this->getGameRecords($startTime, $endTime, 1);
    }

    /**
     * 获取游戏历史记录（单个注单）（适配控制器调用）
     * 
     * @param string $start_time 开始时间，格式：YYYY-MM-DD HH:mm:00
     * @param string $end_time 结束时间，格式：YYYY-MM-DD HH:mm:00
     * @param string $player_id 玩家ID（可选）
    ...
    public function getGameHistory($start_time, $end_time, $player_id = '', $game_code = '', $page = 1, $limit = 100)
    {
        ...
        $result = $this->getGameRecords($startTime, $endTime, $page);
    ...
    /**
     * 将时间字符串规范化为 yyyy-MM-dd HH:mm:ss，解析失败返回空字符串
     */
    private function normalizeToYmdHis($timeStr): string
    {
        if (empty($timeStr)) {
            return '';
        }
