...
export default {
  ...
  methods: {
    ...
    lingqu() {
      let that = this;
      // 确保 nojisuan 是数字类型
      let fanshui = parseFloat(that.nojisuan || 0) || 0;
      if (fanshui <= 0) {
      ...
      that.$apiFun
        ...
        .catch(res => {
          that.$parent.showTost(0, res.message || '领取失败，请重试');
        })
        .finally(() => {
          // 无论成功还是失败都关闭加载状态
      ...
      that.$apiFun
        ...
        .then(res => {
          if (res.code == 200) {
            ...
            // 确保 jisuan 和 nojisuan 是数字类型
            that.jisuan = parseFloat(res.data.jisuan || 0) || 0;
            that.nojisuan = parseFloat(res.data.nojisuan || 0) || 0;
            if (that.page == 1) {
